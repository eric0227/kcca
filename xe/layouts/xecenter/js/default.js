jQuery(function($){
	...
	if(sub_menu_fix == 'yes'){
		$(window).scroll(function(){
			x = $(this).scrollTop();
			if( x > $('#header').height() - $('.menu').height() ){ //스크롤 조건문
				
				if( $('#header').height() == 150 ){  //높이확장기능에 따른 조건문
					setTimeout(function(){	
					$('.lnb').stop().animate({top: x -120}, 1100, 'easeOutBack'); 
					}, 0);
				}
				
				if( $('#header').height() == 50 ){  //높이확장기능에 따른 조건문
					setTimeout(function(){
					$('.lnb').stop().animate({top: x -20}, 1100, 'easeOutBack');
					}, 0);
				}
			}else{  //스크롤 조건문
	...
	//레이아웃 높이 변경
	$('#header').css('height',$.cookie('screen_height_ck')); // 크기 변환후 반환된 쿠키값 적용 (height)
	$('.menu_bottom_wrap').css('marginTop',$.cookie('screen_height_ck2')); // 크기 변환후 반환된 쿠키값 적용 (marginTop)
	
	if($('#header').height() == '150'){ /* 쿠키값에 따른 조건문 (작은값) */
		//기본 문구,이미지 출력
		...
		$('.height_change').toggle(function(){
			$.cookie('screen_height_ck','55px',{expries:1, path:'/'}); /*변경값 쿠키 굽기 */
			$.cookie('screen_height_ck2','10px',{expries:1, path:'/'}); /*변경값 쿠키 굽기 */
			$('#header').animate({height:'55px'},600);
			$('.menu_bottom_wrap').animate({marginTop:'10px'},600);
			...
			$.cookie('screen_height_ck','150px',{expries:1, path:'/'}); /*변경값 쿠키 굽기*/
			$.cookie('screen_height_ck2','100px',{expries:1, path:'/'}); /*변경값 쿠키 굽기*/
			$('#header').animate({height:'150px'},600);
			$('.menu_bottom_wrap').animate({marginTop:'100px'},600);
	...
	if($('#header').height() == '55'){ /* 쿠키값에 따른 조건문 (작은값) */
		//기본 문구,이미지 출력
		...
		$('.height_change').toggle(function(){
			$.cookie('screen_height_ck','150px',{expries:1, path:'/'}); /*변경값 쿠키 굽기*/
			$.cookie('screen_height_ck2','110px',{expries:1, path:'/'}); /*변경값 쿠키 굽기*/
			$('#header').animate({height:'150px'},600);
			$('.menu_bottom_wrap').animate({marginTop:'100px'},600);
			$(this).html('<img src="./layouts/xecenter/img/screen_h1.png"></img>높이확장');
			//이미지변경
