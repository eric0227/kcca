...
<div class="list">
		...
		<fieldset>
			...
			<ul>
				<li class="fbox article">
					...
					<span class="thumb">
						<img src="/kcca/xe/modules/board/skins/xe_2010_gallery/img/noimage.gif" width="<?php echo $__Context->mi->thumbnail_width ?>" height="<?php echo $__Context->mi->thumbnail_height ?>" alt="" /></a>
					</span>
			...
			<ul>
				<?php if($__Context->document_list&&count($__Context->document_list))foreach($__Context->document_list as $__Context->doc){ ?><li class="fbox article <?php echo ($__Context->list_idx++%2)?'odd':'even' ?>">
					...
					<?php }else{ ?>
						<img src="/kcca/xe/modules/board/skins/xe_2010_gallery/img/noimage.gif" width="<?php echo $__Context->mi->thumbnail_width ?>" height="<?php echo $__Context->mi->thumbnail_height ?>" alt="" />
					<?php } ?>
					</span>
					<ul class="flat meta">
	...
	<div class="fbox list-foot">
		<form action="./" method="get" class="pagination"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" />
		<fieldset>
