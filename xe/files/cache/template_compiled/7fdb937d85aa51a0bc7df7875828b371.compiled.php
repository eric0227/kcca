...
echo $__Context->oDocument->getNickName();
	...
	<div class="read_body">
		...
		<form action="./" method="get" onsubmit="return procFilter(this, input_password)"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
			...
			<p><input type="password" name="password" id="cpw" class="iText" /><input class="btn" type="submit" value="<?php echo $__Context->lang->cmd_input ?>" /></p>
		</form>
		<?php }else{ ?>
		...
		<div class="tns">
			...
			<script>
				jQuery(function($){
					$('.twitter>a').snspost({
						type : 'twitter',
						content : '<?php echo $__Context->oDocument->getTitle() ?> <?php echo $__Context->oDocument->getPermanentUrl() ?>'
					});
					$('.me2day>a').snspost({
						type : 'me2day',
						content : '\"<?php echo $__Context->oDocument->getTitle() ?>\":<?php echo $__Context->oDocument->getPermanentUrl() ?>'
					...
					$('.facebook>a').snspost({
						type : 'facebook',
						content : '<?php echo $__Context->oDocument->getTitle() ?>'
					});
					$('.delicious>a').snspost({
						type : 'delicious',
						content : '<?php echo $__Context->oDocument->getTitle() ?>'
...
<?php if($__Context->oDocument->allowTrackback()){ ?>
	<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/xe_2010_gallery','_trackback.html') ?>
<?php } ?>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/xe_2010_gallery','_comment.html') ?>
