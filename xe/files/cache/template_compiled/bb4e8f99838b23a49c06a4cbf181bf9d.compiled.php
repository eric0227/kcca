		...
		<nav class="gnb <?php if($__Context->_COOKIE['__xe_admin_gnb_status'] == 'open'){ ?>open<?php } ?>" id="gnb">
			...
			<?php if($__Context->gnbUrlList&&count($__Context->gnbUrlList))foreach($__Context->gnbUrlList AS $__Context->key=>$__Context->value){ ?>
				...
				<li<?php if($__Context->_COOKIE['__xe_admin_gnb_tx_favorite'] == 'open'){ ?> class="open"<?php } ?>>
					...
					<a href="#favorite" data-href="favorite"><span class="tx"><?php echo $__Context->lang->favorite ?></span></a>
					<ul id="favorite"<?php if($__Context->_COOKIE['__xe_admin_gnb_tx_favorite'] == 'open'){ ?> style="display:block"<?php } ?>>
						<?php if($__Context->favorite_list&&count($__Context->favorite_list))foreach($__Context->favorite_list as $__Context->favorite){ ?><li>
							...
							<form class="remove" action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
...
echo $__Context->value['href'] ?>" data-href="<?php echo md5($__Context->value['href']) ?>"><span class="tx"><?php echo $__Context->value['text'] ?></span></a>
					<?php if(count($__Context->value['list'])){ ?><ul<?php if($__Context->_COOKIE['__xe_admin_gnb_tx_' . md5($__Context->value['href'])] == 'open'){ ?> style="display:block"<?php } ?>>
						<?php if($__Context->value['list']&&count($__Context->value['list']))foreach($__Context->value['list'] as $__Context->key2=>$__Context->value2){ ?><li<?php if($__Context->value2['text'] == $__Context->subMenuTitle){ ?> class="active_"<?php } ?>><a href="<?php echo getFullUrl('');
