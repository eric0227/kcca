...
<!--#Meta:modules/autoinstall/tpl/js/waiting.js--><?php $__tmp=array('modules/autoinstall/tpl/js/waiting.js','','','');Context::loadFile($__tmp,'true','','');unset($__tmp); ?>
...
<div class="x_well">
	<?php if($__Context->package->installed){ ?><p><?php echo $__Context->lang->current_version ?>: <?php echo $__Context->package->cur_version ?> <?php if($__Context->package->need_update){ ?>(<?php echo $__Context->lang->require_update ?>)<?php } ?></p><?php } ?>
	<?php if(!$__Context->package->installed){ ?><p><?php echo $__Context->lang->require_installation ?></p><?php } ?>
	<?php if($__Context->package->depends){ ?>
...
<?php if(!$__Context->package->installed || $__Context->package->need_update){ ?>
	<?php if($__Context->show_ftp_note){ ?><div class="x_well x_clearfix">
	...
	<?php if(!$__Context->show_ftp_note){ ?><div>
		...
		<?php Context::addJsFile("modules/autoinstall/ruleset/ftp.xml", FALSE, "", 0, "body", TRUE, "") ?><form action="./" class="x_form-horizontal" method="post" ><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="ftp" />
			...
			<?php if(!$__Context->need_password){ ?><input type="hidden" name="ftp_password" value="dummy" /><?php } ?>
