	...
	<div id="body">
		...
		<div id="content">
			...
			<form action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
				...
				<input type="hidden" name="use_rewrite" value="<?php echo $__Context->use_rewrite ?>" />
				<div class="x_control-group">
					<select name="time_zone" style="width:100%">
						<?php if($__Context->time_zone&&count($__Context->time_zone))foreach($__Context->time_zone as $__Context->key=>$__Context->val){ ?><option id="<?php echo $__Context->key ?>" value="<?php echo $__Context->key ?>"<?php if($__Context->key==date('O')){ ?> selected="selected"<?php } ?>><?php echo $__Context->val ?></option><?php } ?>
					</select>
