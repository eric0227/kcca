...
Context::addJsFile("modules/layout/ruleset/updateLayout.xml", FALSE, "", 0, "body", TRUE, "") ?><form id="config_form"  action="./" enctype="multipart/form-data" method="post"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="updateLayout" />
...
echo htmlspecialchars($__Context->var->value);
}else{;
...
echo $__Context->var->value;
					...
					<?php if($__Context->var->type == 'image'){ ?>
						...
						<div id="preview_<?php echo $__Context->name ?>" class="x_thumbnail" style="<?php if(!$__Context->var->value){ ?>display:none;<?php } ?>max-width:210px;margin-bottom:20px;">
							<img<?php if($__Context->var->value){ ?> src="/kcca/xe/<?php echo $__Context->var->value ?>"<?php } ?> alt="" style="max-width:100%" />
						...
						<div class="x_thumbnails">
							<?php if($__Context->var->options&&count($__Context->var->options))foreach($__Context->var->options as $__Context->key=>$__Context->val){ ?>
								<div class="x_span2 <?php if($__Context->val->thumbnail){ ?>x_thumbnail<?php } ?>">
									<?php if($__Context->val->thumbnail){ ?><img src="/kcca/xe/<?php echo $__Context->val->thumbnail ?>" alt="<?php echo $__Context->val->val ?>" /><?php } ?>
									<div class="x_caption">
			...
			<?php if($__Context->selected_layout->menu&&count($__Context->selected_layout->menu))foreach($__Context->selected_layout->menu as $__Context->menu_name=>$__Context->menu_info){ ?><div class="x_control-group">
				...
				<div class="x_controls">
					<select name="<?php echo $__Context->menu_name ?>" id="<?php echo $__Context->menu_name ?>">
						<option value="0"><?php echo $__Context->lang->cmd_select ?></option>
						<option<?php if(!$__Context->menu_info->menu_srl||$__Context->menu_info->menu_srl == -1){ ?> selected="selected"<?php } ?> value="-1"><?php echo $__Context->lang->sitemap_with_homemenu ?></option>
	...
	<div class="x_clearfix btnArea">
		<span class="etc">
			<?php if($__Context->layout){ ?><a class="x_btn" href="<?php echo getUrl('', 'module', 'admin', 'act', 'dispLayoutAdminInstanceList', 'layout', $__Context->selected_layout->layout) ?>"><?php echo $__Context->lang->cmd_list ?></a><?php } ?>
			<?php if(!$__Context->layout){ ?><a class="x_btn" href="<?php echo getUrl('', 'module', 'admin', 'act', 'dispLayoutAdminAllInstanceList') ?>"><?php echo $__Context->lang->cmd_list ?></a><?php } ?>
		</span>
