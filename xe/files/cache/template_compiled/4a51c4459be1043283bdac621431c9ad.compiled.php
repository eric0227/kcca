		...
		<div id="content">
			<form method="post" action="./"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
				...
				<?php if(DB::getEnableList()&&count(DB::getEnableList()))foreach(DB::getEnableList() as $__Context->key=>$__Context->val){ ?><div class="x_control-group">
					<label for="db_type_<?php echo $__Context->val->db_type ?>"><input name="db_type" type="radio" value="<?php echo $__Context->val->db_type ?>"<?php if(!$__Context->val->enable){ ?> disabled="disabled"<?php } ?> id="db_type_<?php echo $__Context->val->db_type ?>"<?php if($__Context->val->db_type==$__Context->defaultDatabase){ ?> checked="checked"<?php } ?> /> <?php echo $__Context->val->db_type ?></label>
				...
				<?php if(DB::getDisableList()&&count(DB::getDisableList()))foreach(DB::getDisableList() as $__Context->key=>$__Context->val){ ?><div class="x_control-group x_muted">
					<label for="db_type_<?php echo $__Context->val->db_type ?>"><input name="db_type" type="radio" value="<?php echo $__Context->val->db_type ?>"<?php if(!$__Context->val->enable){ ?> disabled="disabled"<?php } ?> id="db_type_<?php echo $__Context->val->db_type ?>" /> <?php echo $__Context->val->db_type ?><em>(<?php echo $__Context->lang->can_use_when_installed ?>)</em></label>
					<p><?php echo $__Context->lang->db_desc[$__Context->val->db_type] ?></p>
				</div><?php } ?>
				<div class="ibtnArea">
					<span class="x_pull-left">
						<a href="<?php echo getUrl('', 'act', 'dispInstallCheckEnv') ?>" class="x_btn x_btn-small x_btn-inverse"><i class="x_icon-chevron-left x_icon-white"></i> <?php echo $__Context->lang->cmd_back ?></a>
					</span>
