...
<table class="x_table x_table-striped x_table-hover">
	...
	<tbody>
		...
		<?php if($__Context->layout&&count($__Context->layout))foreach($__Context->layout as $__Context->no=>$__Context->item){ ?><tr>
			<?php if($__Context->no === 0){ ?><td rowspan="<?php echo count($__Context->layout) ?>" ><?php echo $__Context->count++ ?></td><?php } ?>
			<?php if($__Context->no === 0){ ?><td rowspan="<?php echo count($__Context->layout) ?>" ><?php echo $__Context->layout_name ?></td><?php } ?>
			<td><?php echo $__Context->item->title ?></td>
			<td><?php echo zdate($__Context->item->regdate, "Y-m-d") ?></td>
			<td><a href="<?php echo getUrl('act', 'dispLayoutAdminModify', 'layout_srl', $__Context->item->layout_srl) ?>"><?php echo $__Context->lang->cmd_setup ?></a></td>
			<td><a href="<?php echo getUrl('act', 'dispLayoutAdminEdit', 'layout_srl', $__Context->item->layout_srl) ?>"><?php echo $__Context->lang->cmd_edit ?></a></td>
			<td><a href="<?php echo getUrl('', 'module', 'layout', 'act', 'dispLayoutAdminCopyLayout', 'layout_srl', $__Context->item->layout_srl) ?>" onclick="popopen(this.href);return false;" title="<?php echo $__Context->lang->cmd_copy ?>"><?php echo $__Context->lang->cmd_copy ?></a></td>
			<td>
