<?php if(!defined("__XE__"))exit;?>	</div>
	...
	<footer class="footer">
		<p class="power">
			...
			<?php if(isset($__Context->latestVersion)){ ?><strong>Latest version</strong>: <a href="<?php echo htmlspecialchars(html_entity_decode($__Context->latestVersion->url)) ?>" target="_blank" title="<?php echo zdate($__Context->latestVersion->date, 'Y-m-d') ?>"><?php echo $__Context->latestVersion->title ?></a><?php } ?>
		</p>
		<p class="cache">
			...
			<a href="http://code.google.com/p/xe-core/issues/entry" target="_blank" style="vertical-align:middle"><?php echo $__Context->lang->bug_report ?></a>
		</p>
	</footer>
</div>
<script>
