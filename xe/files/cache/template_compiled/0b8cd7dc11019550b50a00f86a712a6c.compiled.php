...
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/install/tpl','header.html') ?>
	<div id="body">
		...
		<div id="content">
			...
			<form rule="mysql" action="./" method="post" class="x_form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
				...
				<h2><?php echo $__Context->title ?></h2>
				...
				<div class="ibtnArea">
					<span class="x_pull-left">
						<a href="<?php echo getUrl('', 'act', 'dispInstallSelectDB') ?>" class="x_btn x_btn-small x_btn-inverse"><i class="x_icon-chevron-left x_icon-white"></i> <?php echo $__Context->lang->cmd_back ?></a>
					</span>
					<div class="x_pull-right">
						<button type="submit" class="x_btn x_btn-small x_btn-inverse" id="task-db-info"><?php echo $__Context->lang->cmd_next ?> <i class="x_icon-chevron-right x_icon-white"></i></button>
					</div>
				</div>
			</form>
		</div>
