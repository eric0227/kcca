<?php if(!defined("__XE__"))exit;?><!--#Meta:modules/member/tpl/js/member_admin_group.js--><?php $__tmp=array('modules/member/tpl/js/member_admin_group.js','','','');Context::loadFile($__tmp,'true','','');unset($__tmp); ?>
<script>
...
<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/member/tpl/group_list/1'){ ?><div class="message <?php echo $__Context->XE_VALIDATOR_MESSAGE_TYPE ?>">
	<p><?php echo $__Context->XE_VALIDATOR_MESSAGE ?></p>
</div><?php } ?>
<?php Context::addJsFile("modules/member/ruleset/insertGroupConfig.xml", FALSE, "", 0, "body", TRUE, "") ?><form action="" method="post" ><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertGroupConfig" />
			...
			<?php if($__Context->group_list&&count($__Context->group_list))foreach($__Context->group_list as $__Context->group_srl=>$__Context->group_info){ ?><tr>
				<td>
					<div class="wrap">
						...
						<input type="text" name="group_titles[]" value="<?php echo htmlspecialchars($__Context->group_info->title) ?>" class="lang_code" title="<?php echo $__Context->lang->group_title ?>" />
				...
				<td><input type="text" name="descriptions[]" value="<?php echo $__Context->group_info->description ?>" title="<?php echo $__Context->lang->description ?>" /></td>
				...
				<td class="_imageMarkButton">
					...
					<?php if($__Context->config->group_image_mark == 'Y' && $__Context->group_info->image_mark){ ?><img src="<?php echo $__Context->group_info->image_mark ?>" alt="<?php echo htmlspecialchars($__Context->group_info->title) ?>" /><?php } ?>
					<a href="#imageMark" class="modalAnchor _imageMark filebox"><?php echo $__Context->lang->cmd_modify ?></a>
				</td>
				<td><div class="_deleteTD"<?php if($__Context->group_info->is_default == 'Y'){ ?> style="display:none"<?php } ?>><a href="#<?php echo $__Context->group_srl ?>" class="_deleteGroup"><?php echo $__Context->lang->cmd_delete ?></a></div></td>
			</tr><?php } ?>
...
<section class="x_modal" id="imageMark">
	...
	<div class="x_modal-footer">
		...
		<p class="x_pull-right"><?php echo $__Context->lang->add_group_image_mark ?>: <a href="<?php echo getUrl('', 'module','admin', 'act', 'dispModuleAdminFileBox') ?>" target="_blank"><?php echo $__Context->lang->link_file_box ?></a></p>
	</div>
</section>
