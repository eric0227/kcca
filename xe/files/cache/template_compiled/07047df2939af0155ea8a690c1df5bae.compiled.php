<?php if(!defined("__XE__"))exit;?><section class="section">
	...
	<?php Context::addJsFile("modules/rss/ruleset/insertRssModuleConfig.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" class="x_form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertRssModuleConfig" />
		...
		<div class="x_control-group">
			...
			<div class="x_controls">
				<select name="open_rss" id="open_rss">
					<?php if($__Context->lang->open_rss_types&&count($__Context->lang->open_rss_types))foreach($__Context->lang->open_rss_types as $__Context->key=>$__Context->val){ ?><option value="<?php echo $__Context->key ?>"<?php if((!$__Context->rss_config->open_rss && $__Context->key == 'N') || ($__Context->rss_config->open_rss == $__Context->key)){ ?> selected="selected"<?php } ?>><?php echo $__Context->val ?></option><?php } ?>
				</select>
			</div>
		</div>
		<div class="x_control-group">
		...
		<div class="x_control-group">
			...
			<div class="x_controls">
				<textarea name="feed_description" id="feed_description" rows="4" cols="42" style="float:left;margin-right:8px"><?php echo htmlspecialchars($__Context->rss_config->feed_description) ?></textarea>
		...
		<div class="x_control-group">
			...
			<div class="x_controls">
				<textarea name="feed_copyright" id="feed_copyright" rows="4" cols="42" style="float:left;margin-right:8px"><?php echo htmlspecialchars($__Context->rss_config->feed_copyright) ?></textarea>
