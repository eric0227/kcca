...
<?php Context::addJsFile("modules/module/ruleset/insertModulesGrant.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" id="manageSelectedModuleGrant" class="section x_form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertModulesGrant" />
	...
	<?php if($__Context->grant_list&&count($__Context->grant_list))foreach($__Context->grant_list as $__Context->grant_name=>$__Context->grant_item){ ?><div class="x_control-group">
		...
		<div class="x_controls">
			...
			<div id="zone_<?php echo $__Context->grant_name ?>" hidden style="margin-top:8px">
				<?php if($__Context->group_list&&count($__Context->group_list))foreach($__Context->group_list as $__Context->group_srl=>$__Context->group_item){ ?><label for="grant_<?php echo $__Context->grant_name ?>_<?php echo $__Context->group_srl ?>">
