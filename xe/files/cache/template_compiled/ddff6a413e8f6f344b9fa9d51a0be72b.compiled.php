...
<!--#Meta:modules/layout/tpl/js/adminEdit.js--><?php $__tmp=array('modules/layout/tpl/js/adminEdit.js','','','');Context::loadFile($__tmp,'true','','');unset($__tmp); ?>
<p><?php echo nl2br($__Context->lang->about_layout_code) ?></p>
...
echo $__Context->file ?></p>
					<form action="./" method="post" class="x_pull-right" style="margin:0;padding:0"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/layout/tpl/layout_edit/1'){ ?><div class="message <?php echo $__Context->XE_VALIDATOR_MESSAGE_TYPE ?>">
		<p><?php echo $__Context->XE_VALIDATOR_MESSAGE ?></p>
	</div><?php } ?>
	<?php Context::addJsFile("modules/layout/ruleset/imageUpload.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" enctype="multipart/form-data"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="imageUpload" />
		<input type="hidden" name="module" value="layout" />
		...
		<p>
			...
			<span class="x_help-inline"><?php echo $__Context->lang->msg_layout_image_target ?></span>
		</p>
...
<?php } ?>
<?php Context::addJsFile("modules/layout/ruleset/codeUpdate.xml", FALSE, "", 0, "body", TRUE, "") ?><form id="fo_layout"  action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="codeUpdate" />
	<input type="hidden" name="module" value="layout" />
	<input type="hidden" name="layout_srl" value="<?php echo $__Context->layout_srl ?>" />
	...
	<section class="section">
		...
		<div>
			<?php if($__Context->widget_list&&count($__Context->widget_list))foreach($__Context->widget_list as $__Context->widget){ ?> <i>|</i> <a href="<?php echo getUrl('','module','admin','act','dispWidgetAdminGenerateCode','selected_widget',$__Context->widget->widget) ?>" target="_blank"><?php echo $__Context->widget->title ?></a><?php } ?>
		</div>
	</section>
	<section class="section">
		<h1>CSS</h1>
	...
	<div class="x_clearfix btnArea">
		...
		<span class="x_pull-right x_btn-group">
			<button type="button" class="x_btn" onclick="doPreviewLayoutCode()"><?php echo $__Context->lang->cmd_preview ?></button>
