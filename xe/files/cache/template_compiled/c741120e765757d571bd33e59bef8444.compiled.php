<?php if(!defined("__XE__"))exit;?><script src="/kcca/xe/modules/board/tpl/js/board_admin.js"></script>
<script src="/kcca/xe/modules/board/tpl/modules/module/tpl/js/module_admin.js"></script>
<script>
</script>
<?php Context::addJsFile("modules/board/ruleset/insertBoardForBasic.xml", FALSE, "", 0, "body", TRUE, "") ?><form  id="fo_obj" class="x_form-horizontal" action="./" method="post" enctype="multipart/form-data"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertBoardForBasic" />
	<input type="hidden" name="act" value="procBoardAdminUpdateBoardFroBasic" />
	<input type="hidden" name="board_name" value="<?php echo $__Context->module_info->mid ?>" />
...
echo $__Context->val->user_id;
	...
	<p style="text-align:right;border-top:1px dotted #ddd;padding:10px 0"><a href="<?php echo $__Context->setupUrl ?>" style="text-decoration:underline"><?php echo $__Context->lang->advanced_settings ?></a></p>
</form>
