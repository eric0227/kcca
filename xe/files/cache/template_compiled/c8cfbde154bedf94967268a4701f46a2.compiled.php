...
<?php }else{ ?>
    ...
    <?php if($__Context->output->data&&count($__Context->output->data))foreach($__Context->output->data as $__Context->no => $__Context->trackback){ ?>
        <li>
            <dl>
                <dt><a href="<?php echo getUrl('','document_srl',$__Context->trackback->document_srl) ?>" onclick="window.open(this.href);return false;"><?php echo htmlspecialchars($__Context->trackback->title) ?></a> </span></dt>
                <dd><?php echo cut_str(htmlspecialchars($__Context->trackback->excerpt)) ?></dd>
            </dl>
            <address><strong><a href="<?php echo $__Context->trackback->url ?>"><?php echo htmlspecialchars($__Context->trackback->blog_name) ?></a></strong> | <span class="time"><?php echo zdate($__Context->trackback->regdate, "Y-m-d H:i") ?></span> </address>
        </li>
    <?php } ?>
    </ul>
