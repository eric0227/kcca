...
<form action="./" method="post" enctype="multipart/form-data" class="x_form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" />
			...
			<div class="x_controls">
				<?php if($__Context->skin_info->colorset&&count($__Context->skin_info->colorset))foreach($__Context->skin_info->colorset as $__Context->key=>$__Context->val){ ?>
					...
					<div<?php if($__Context->val->screenshot){ ?> class="x_thumbnail"<?php } ?> style="display:inline-block;width:<?php echo $__Context->_width ?>px;margin-right:10px;">
						...
						<?php if($__Context->val->screenshot){ ?><img src="/kcca/xe/<?php echo $__Context->val->screenshot ?>" alt="<?php echo $__Context->val->title ?>" /><?php } ?>
					</div>
				<?php } ?>
			</div>
		</div><?php } ?>
...
echo htmlspecialchars($__Context->val->value);
} ?>" class="lang_code" /><?php } ?>
