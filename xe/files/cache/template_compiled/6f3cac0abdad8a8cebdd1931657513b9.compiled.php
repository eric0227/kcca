...
<form action="./" method="post" onsubmit="return procFilter(this, window.insert)" class="board_write"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<div class="write_footer">
		...
		<div class="write_author">
			<?php if(!$__Context->is_logged){ ?><span class="item">
				...
				<input type="text" name="nick_name" id="userName" class="iText userName" style="width:80px" value="<?php echo $__Context->oDocument->getNickName() ?>" />
			</span><?php } ?>
			<?php if(!$__Context->is_logged){ ?><span class="item">
			...
			<span class="item">
				...
				<input type="text" name="tags" id="tags" value="<?php echo htmlspecialchars($__Context->oDocument->get('tags')) ?>" class="iText" style="width:260px" title="Tag" />
			</span>		
		</div>
		<div class="btnArea">
