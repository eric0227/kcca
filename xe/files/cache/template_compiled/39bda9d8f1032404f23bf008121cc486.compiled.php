	...
	<div<?php if(!$__Context->use_in_page){ ?> class="x_modal-body"<?php } ?>>
		<div class="x_tabbable">
			...
			<div class="x_tab-content">
				<form action="" class="x_tab-pane <?php if(!$__Context->use_in_page){ ?>x_active<?php } ?> item" id="lang_create"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment()) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
					<fieldset class="editMode">
						<?php if($__Context->lang_supported&&count($__Context->lang_supported))foreach($__Context->lang_supported as $__Context->code=>$__Context->name){ ?><textarea class="<?php echo $__Context->code ?>" rows="1" cols="12" title="<?php echo $__Context->name ?>" style="margin-right:5px"></textarea><?php } ?>
